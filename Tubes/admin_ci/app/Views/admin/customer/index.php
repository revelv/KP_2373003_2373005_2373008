...
<form action="<?= base_url('admin/customer/search') ?>" method="get" style="margin-bottom:20px; display:flex; gap:10px; flex-wrap:wrap;">
    ...
    <select name="filter" style="flex:1;">
        ...
        <option value="provinsi" <?= ($filter ?? '') === 'provinsi' ? 'selected' : '' ?>>Provinsi</option> 
        <option value="kota" <?= ($filter ?? '') === 'kota' ? 'selected' : '' ?>>Kota</option>
        <option value="kecamatan" <?= ($filter ?? '') === 'kecamatan' ? 'selected' : '' ?>>Kecamatan</option> </select>
        ...
        <?php if (!empty($customers)): ?>
            <?php foreach ($customers as $c): ?>
                <tr>
                    ...
                    <td><?= esc($c['provinsi']) ?></td> 
                    <td><?= esc($c['kota']) ?></td>
                    <td><?= esc($c['kecamatan']) ?></td> <td><?= esc($c['alamat']) ?></td>
                    <td>
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <tr><td colspan="9" style="text-align:center;">Tidak ada data customer yang ditemukan.</td></tr>
        <?php endif; ?>
    </tbody>
</table>
<?= $this->endSection() ?>