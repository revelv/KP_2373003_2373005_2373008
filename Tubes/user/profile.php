...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['profile_image']['name']) && is_uploaded_file($_FILES['profile_image']['tmp_name'])) {
        ...
        $uploadDirFs  = __DIR__ . '/../uploads/profile/';
        $uploadDirWeb = '../uploads/profile/';
    ...
    <form method="post" action="profile.php" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input
                ...
                id="postal_code"
                name="postal_code"
                value="<?= htmlspecialchars((string)$postal_code) ?>"
                required
                readonly>
            <div class="form-text">Kode pos terisi otomatis berdasarkan kelurahan.</div>
        </div>
...
<script>
    ...
    const currentPostal   = <?= json_encode((string)$postal_code) ?>;
    ...
    async function loadSubDistricts(districtId, autoSelect = false) {
        ...
        try {
            const res = await fetch('./rajaongkir/get-subdistrict.php?district=' + encodeURIComponent(districtId) + '&t=' + Date.now());
            const txt = await res.text();
            let data;
            try { data = JSON.parse(txt); }
            catch (e) {
                console.error('Subdistrict JSON parse error:', e, txt);
                kelSel.innerHTML = '<option value="">Gagal load kelurahan</option>';
                kelSel.disabled = true;
                return;
            }

            // Expect { success:true, data:[...] } tapi tetap fleksibel
            let list = [];
            ...
            const options = list.map(s => {
                ...
                const postal = String(
                    s.postal_code ?? s.postcode ?? s.zip_code ?? s.zipcode ?? s.zip ?? ''
                ).trim();
                if (!id || !name) return '';
    ...
    document.addEventListener('change', (e) => {
        ...
        if (t === kelSel) {
            ...
            if (postalInp) postalInp.value = postal;
        }
    });
